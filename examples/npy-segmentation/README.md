...
data = {"path": str(DATA.parent), "train": "images/train", "val": "images/val", "test": "images/test", "channels": 1, "names": {0: "positive", 1: "negative"}, "npy_padding_value": PAD}
...
model.train(data=str(DATA), epochs=1, imgsz=256, batch=2, cache=False, npy_padding_value=PAD,
            mosaic=0.0, mixup=0.0, cutmix=0.0, copy_paste=0.0, hsv_h=0.0, hsv_s=0.0, hsv_v=0.0,
...
# Prediction
for source in ["/path/to/sample.npy", ["/path/to/a.npy", "/path/to/b.npy"], "/path/to/PRACTICE_YOLO_SEG_NPY/images/test"]:
    for r in best.predict(source=source, npy_padding_value=PAD):
        print(r.path, r.boxes, None if r.boxes is None else r.boxes.cls, None if r.boxes is None else r.boxes.conf, r.masks)
```
